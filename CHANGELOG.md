...
### Changed
- **Self-heal a stopped Docker daemon on resume/pull** — resume now starts Docker (Docker Desktop on macOS/Windows, the docker service on Linux) and waits for it **before preflight**, so a down daemon no longer hard-blocks (previously preflight failed before the container step could start Docker). Container `up()` keeps an idempotent check as a backstop. After the container starts, resume waits for the database to accept connections before migrations/restore.
