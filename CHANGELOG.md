...
### Changed
- **Self-heal a stopped Docker daemon on resume/pull** — bringing a container up now checks the Docker daemon first and, if it's down, starts Docker Desktop (macOS/Windows) or the docker service (Linux) and waits for it to be ready instead of failing with "Is the docker daemon running?". After the container starts, resume also waits for the database to accept connections before running migrations/restore, so a just-started Postgres/MySQL doesn't cause "cannot connect".
